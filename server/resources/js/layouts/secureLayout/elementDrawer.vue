<template>
  <div v-if="$route.name == 'Formbuilder'">
    ...
    >
      ...
      >
        <v-tab> Element </v-tab>
        <v-tab :disabled="paramsLength == 0"> Parametrar </v-tab>
      </v-tabs>
      ...
      <base-layout-card transparent :level="4">
        <v-tabs-items v-model="activeTab">
          ...
          <v-tab-item>
            ...
            >
              <v-card-text
                v-if="Object.keys($store.state.elements.params).length > 0"
              >
                <v-btn
                  depressed
                  ...
                  color="red lighten-2"
                  @click="
                    removeElement(elementId, elementType, elementDescription)
                  "
                >
                ...
                <template v-if="['expansionPanel'].includes(elementType)">
                  <template v-for="(text, i) in elementParams.multipleElements">
                    <span :key="i" class="white--text">
                      Expansion panel {{ i + 1 }} text
                    </span>
                    <v-textarea
                      :key="i"
                      v-model="elementParams.multipleElementsParams[i]"
                      v-lazy-input:debounce="200"
                      class="mb-n4"
                      dense
                      solo
                ...
                <template v-if="['text', 'button'].includes(elementType)">
                  ...
                  <v-row class="mr-1 mt-n5" justify="end">
                    <v-btn
                      ...
                      class="ml-2"
                      text
                      @click.stop="elementParams.fontSize = 17"
                    >
                      Återställ
                    </v-btn>
                  </v-row>
                ...
                <template v-if="elementType === 'text'">
                  ...
                  <v-text-field
                    v-model="elementParams.lineHeight"
                    v-lazy-input:debounce="200"
                    step=".1"
                    color="white"
                    solo
                ...
                <template v-if="['googleMap'].includes(elementType)">
                  <v-checkbox
                    v-model="elementParams.viewControl"
                ...
                <template v-if="['button'].includes(elementType)">
                  <v-checkbox
                    v-model="elementParams.showBorder"
                    class="mt-n2"
                    :ripple="false"
                    dark
                  >
                ...
                <template v-if="['button'].includes(elementType)">
                  ...
                  <v-autocomplete
                    ...
                    :disabled="
                      elementParams.internalLink || elementParams.externalLink
                        ? true
                        : false
                    "
                    dense
                ...
                <template v-if="['button', 'image'].includes(elementType)">
                  ...
                  <v-select
                    ...
                    :disabled="
                      elementParams.specialAction.length ||
                      elementParams.externalLink
                        ? true
                        : false
                    "
                    clearable
                ...
                <template v-if="['layout'].includes(elementType)">
                  ...
                  <v-text-field
                    v-model="elementParams.backgroundColor"
                    v-lazy-input:debounce="200"
                    dense
                    solo
                    background-color="#edefef"
                  >
                    <template v-slot:append>
                      <v-icon
                        small
                        class="mt-n1 mr-2"
                        :color="elementParams.backgroundColor"
                      >
                        mdi-circle
                      </v-icon>
                ...
                <template v-if="['radioGroup'].includes(elementType)">
                  <v-checkbox
                    v-model="elementParams.radioColumn"
                    class="mt-n3"
                    dark
                ...
                >
                  <v-row no-gutters class="my-n4 mb-n2">
                    ...
                    <v-col :sm="elementType == 'layout' ? '12' : '4'">
                      ...
                      <v-text-field
                        v-model="elementParams.heightSm"
                        v-lazy-input:debounce="200"
                        class="mr-1"
                        dense
                        solo
                    ...
                    <v-col v-if="elementType != 'layout'" sm="4">
                      ...
                      <v-text-field
                        v-model="elementParams.heightMd"
                        v-lazy-input:debounce="200"
                        class="mr-1"
                        dense
                        solo
                    ...
                    <v-col v-if="elementType != 'layout'" sm="4">
                      ...
                      <v-text-field
                        v-model="elementParams.heightLg"
                        v-lazy-input:debounce="200"
                        dense
                        solo
                        background-color="#edefef"
                ...
                <template v-if="['image'].includes(elementType)">
                  ...
                  <v-row class="mr-1 mt-n1" justify="end">
                    <v-btn
                      ...
                      class="ml-5 mt-n3"
                      text
                      @click.stop="elementParams.creditAlign = 'start'"
                    >
                      Left
                    </v-btn>
                    <v-btn
                      ...
                      class="ml-2 mt-n3"
                      text
                      @click.stop="elementParams.creditAlign = 'center'"
                    >
                      Center
                    </v-btn>
                    <v-btn
                      ...
                      class="ml-2 mt-n3"
                      text
                      @click.stop="elementParams.creditAlign = 'end'"
                    >
                      Right
                    </v-btn>
                  </v-row>
                  <v-text-field
                    v-model="elementParams.image.metadata.credits"
                    dense
                    solo
                    background-color="#edefef"
                  />
                ...
                <template v-if="['image', 'video'].includes(elementType)">
                  ...
                  <v-row class="mr-1 mt-n5" justify="end">
                    <v-btn
                      ...
                      class="ml-2"
                      text
                      @click.stop="elementParams.width = '100%'"
                    >
                      Återställ
                    </v-btn>
                  </v-row>
                  <v-text-field
                    v-model="elementParams.width"
                    v-lazy-input:debounce="200"
                    dense
                    solo
                    background-color="#edefef"
                  ...
                  >
                    <template v-slot:append>
                      ...
                      <v-icon
                        ...
                        @click="dialogBackgroundColorPicker = true"
                      >
                        mdi-format-color-fill
                      </v-icon>
                    </template>
                ...
                <template v-if="['imageCarousel'].includes(elementType)">
                  ...
                  <div class="mt-n10">
                    <v-checkbox
                      v-model="elementParams.hideDelimiters"
                      :ripple="false"
                      dark
                      label="Göm indikationspunkter"
                      hide-details
                    >
                    </v-checkbox>
                    <v-checkbox
                    ...
                    <v-checkbox
                      v-model="elementParams.showArrows"
                      :ripple="false"
                      dark
                      label="Göm pilar"
                      hide-details
                  ...
                  <template v-if="elementType == 'layout'">
                    ...
                    <v-row class="mr-1 mt-n1" justify="end">
                      <v-btn
                        ...
                        text
                        @click.stop="
                          (elementParams.cropLeft = 0),
                            (elementParams.cropRight = 0),
                            (elementParams.cropTop = 0),
                            elementParams.cropBottom
                        "
                      >
                        Återställ
                      </v-btn>
      ...
      <v-footer color="rgba(43, 54, 67, 0.7)" absolute>
        ...
        <v-icon
          ...
          class="ml-3"
          style="cursor: pointer"
          @click="changeElementFramesVisibility()"
        >
          mdi-eye
        </v-icon>
      </v-footer>
    ...
    <v-dialog
      v-model="dialogColorPickerElementColor"
      width="300px"
      :transition="false"
    ...
    >
      <v-color-picker
        v-if="dialogColorPickerElementColor"
        v-model="elementParams.color"
    ...
    <v-dialog
      v-model="dialogColorPickerGridColor"
      width="300px"
      :transition="false"
      style="overflow: hidden"
      hide-overlay
    >
      <v-color-picker
        v-if="dialogColorPickerGridColor"
        v-model="elementParams.backgroundColor"
        v-lazy-input:debounce="100"
        mode="hexa"
      />
    ...
    <v-dialog
      v-model="dialogBackgroundColorPicker"
      width="300px"
      :transition="false"
      style="overflow: hidden"
      hide-overlay
    >
      <v-color-picker
        v-if="dialogBackgroundColorPicker"
        v-model="elementParams.backgroundColor"
        v-lazy-input:debounce="100"
        mode="hexa"
      />
...
export default {
  ...
  watch: {
    "elementParams.specialAction"(val, newVal) {
      if (!val) return;
      return (
        (this.elementParams.externalLink = ""),
        (this.elementParams.internalLink = undefined)
      );
    },
    ...
    "elementParams.internalLink"(val, newVal) {
      ...
      return (
        (this.elementParams.externalLink = ""),
        (this.elementParams.specialAction = "")
      );
    },
  ...
  methods: {
    ...
    checkEmptyMultipleElements() {
      if (this.elementParams.multipleElements.length === 0) {
