...
<script>
  ...
  export default {
    ...
    computed: {
      ...sync('theme', ['isDark']),
      ...sync('modulesManagement', ['dialogCapability', 'capability', 'editingCapability']),
      ...get('modulesManagement', ['selectedModule']),

      addPrefixToName() {
        return `${this.selectedModule.name}.${this.capability.name}`;
      },
    },
          ...
          if (validated) {
            ...
            if (this.editingCapability) {
              this.editCapabilitySaveChanges(editedCapability).then((edited) => {
                if (edited) {
                  this.dialogCapability = false;
                  this.capability.name = `${this.selectedModule.name}.${this.capability.name}`;
                  this.editingCapability = false;
                }
              });
            } else {
              this.createCapability(newCapabilityPrefixed).then((created) => {
                if (created) {
                  this.selectedModule.capabilities.push(newCapabilityPrefixed);
                  this.dialogCapability = false;
                  this.capability = {};
                }
              });
