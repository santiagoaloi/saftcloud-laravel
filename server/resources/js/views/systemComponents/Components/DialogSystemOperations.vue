<template>
 <baseDialog v-model="internalValue" title="Advanced operations" persistent max-width="900" show-remove @close="() => (internalValue = false)">
  <v-card color="#f8f9fa">
   <v-card-text>
    <v-container>
     <v-list color="transparent">
      <v-list-item>
       ...
       <v-list-item-content>
        <v-list-item-title>Refresh all component configuration structure</v-list-item-title>
        <v-list-item-subtitle>
         Update v_components config column for all components with new structure. This will remove old keys, rename old values and compare object
         structures with template structures to keep consistency. This might require re-configuration in components where certain configurations won't
         be valid anymore.
        </v-list-item-subtitle>
        <v-list-item-subtitle>
...
export default {
 ...
 methods: {
  ...
  compareAndModifyObject(object, template) {
   (function addFromTemplate(obj, temp) {
    for (var prop in temp) {
     ...
     else if (typeof obj[prop] == "object" && typeof temp[prop] == "object") addFromTemplate(obj[prop], temp[prop]); // Nested objects! Recursion-step
    }
   ...
   (function removeFromObject(obj, temp) {
    for (var prop in obj) {
     ...
     else if (typeof obj[prop] == "object" && typeof temp[prop] == "object") removeFromObject(obj[prop], temp[prop]); // Nested objects! Recursion-step
    }
   ...
   this.$swal({
    ...
    if (result.value) {
     ...
     this.updatedComponentList.forEach(component => {
      ...
      this.compareAndModifyObject(component.config.componentConfig, this.componentConfigStructure);
  ...
  save() {
   axios.post("sximo/components/saveAllComponentConfig/", this.updatedComponentList).then(response => {
    if (response.data.status == "success") {
     window.getApp.$emit("APP_SHOW_SNACKBAR", "teal accent-4", "mdi-information-variant", "Component settings saved.");
    }
   });
  }
 }
